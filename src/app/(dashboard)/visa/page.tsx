...
export default function VisaPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      ...
      <PageHeader
        title="Visa Guidance & Simulator"
        subtitle="Embassy checksheets, visa probability meters, and AI mock interviews."
      />
      ...
      <div className="flex flex-wrap gap-2 border-b border-border/40 pb-4">
        {COUNTRIES.map(c => (
          <button 
            key={c} 
            type="button"
            onClick={() => { setSelected(c); resetInterview(); setQIndex(0) }}
      ...
      {visa && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2 bg-card border border-border rounded-xl p-5 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs border-t border-border/40 pt-4">
                <div>
                  <p className="text-muted-foreground font-semibold flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> Processing</p>
                  <p className="font-bold text-foreground mt-0.5">{visa.processing_days_min}-{visa.processing_days_max} days</p>
                </div>
                <div>
                  <p className="text-muted-foreground font-semibold flex items-center gap-1"><DollarSign className="w-3.5 h-3.5" /> Visa Fee</p>
                  <p className="font-bold text-foreground mt-0.5">${visa.estimated_cost_usd}</p>
                </div>
                <div>
                  <p className="text-muted-foreground font-semibold flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5" /> Finances</p>
            ...
            {/* Probability Card */}
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 text-white flex flex-col justify-between shadow-xs">
              <div>
                <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Visa Success Probability</p>
                <div className="flex items-baseline gap-1 mt-2">
                  <span className="text-4xl font-extrabold">{prob}%</span>
                  <span className="text-xs text-zinc-300">Confidence</span>
                </div>
              </div>
              <div className="w-full bg-zinc-850 h-2 rounded-full overflow-hidden mt-3">
                <div 
                  className={`h-full rounded-full transition-all duration-750 ${
                    prob >= 80 ? 'bg-emerald-500' : prob >= 60 ? 'bg-amber-500' : 'bg-rose-500'
                  }`} 
                  style={{ width: `${prob}%` }} 
                />
              </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-xs space-y-4">
            <h3 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5 border-b border-border/40 pb-2">
            ...
            {!feedback ? (
              <div className="space-y-4">
                <div className="bg-indigo-500/5 p-4 rounded-xl border border-indigo-500/15">
                ...
                <div className="space-y-1">
                  ...
                  <textarea 
                    ...
                    className="w-full h-24 bg-background border border-border rounded-xl p-3 text-sm text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-indigo-600 dark:focus-visible:ring-indigo-500 focus-visible:border-indigo-600 dark:focus-visible:border-indigo-500 leading-relaxed resize-none transition-shadow"
                  />
                </div>
                ...
                <div className="flex gap-2">
                  {qIndex > 0 && (
                    <button 
                      ...
                      className="px-3.5 py-2 border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-muted text-xs font-semibold rounded-lg transition-colors cursor-pointer"
                    >
                      Prev Question
                    </button>
                  )}
                  {qIndex < questions.length - 1 && (
                    <button 
                      type="button"
                      onClick={() => { setQIndex(qIndex + 1); setAnswer('') }} 
                      className="px-3.5 py-2 border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-muted text-xs font-semibold rounded-lg transition-colors cursor-pointer"
                    >
                      Skip / Next
                    </button>
                  ...
                  <button 
                    ...
                    className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5 disabled:opacity-50"
                  >
              ...
              <div className="space-y-4">
                {/* Result header */}
                <div className="flex items-center justify-between p-4 bg-muted/60 border border-border rounded-xl">
                  <div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-rose-600 dark:text-rose-455 bg-rose-500/5 border border-rose-500/15 rounded-xl p-4 leading-relaxed">
                    {feedback.critique}
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-xs text-emerald-800 dark:text-emerald-300 bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-4 leading-relaxed whitespace-pre-wrap font-medium">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="flex-1 py-2 border border-border text-muted-foreground hover:text-foreground hover:bg-muted text-xs font-semibold rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1"
                  >
                  ...
                  {qIndex < questions.length - 1 && (
                    <button 
                      type="button"
                      onClick={() => { setQIndex(qIndex + 1); resetInterview() }} 
                      className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-xs transition-colors cursor-pointer"
                    >
                      Next Question
                    </button>
                  )}
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-xs space-y-3">
            <h3 className="text-sm font-bold text-foreground uppercase tracking-wider border-b border-border/40 pb-2">Required Checklist Documents</h3>
          ...
          {visa.interview_required && (
            <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-5 shadow-xs space-y-1.5">
              ...
              <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed font-semibold">{visa.interview_tips}</p>
            </div>
          )}

          {/* Rejection reasons */}
          <div className="bg-rose-500/5 border border-rose-500/15 rounded-xl p-5 shadow-xs space-y-2.5">
            <h3 className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider">Common Embassy Rejection Reasons</h3>
            <ul className="space-y-1.5">
              {visa.common_rejection_reasons?.map((reason, i) => (
                <li key={i} className="flex items-start gap-1.5 text-xs text-rose-700 dark:text-rose-350 leading-relaxed font-semibold">
