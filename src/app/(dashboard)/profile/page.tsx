...
const inputClass = "w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white"
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2">
        <Field label="Full name">
          <input className={inputClass} value={data.full_name}
            onChange={e => update('full_name', e.target.value)} placeholder="Rahul Kumar" />
        </Field>
      </div>
      <Field label="Branch / stream">
        <select className={selectClass} value={data.branch} onChange={e => update('branch', e.target.value)}>
          <option value="">Select branch</option>
          {BRANCHES.map(b => <option key={b}>{b}</option>)}
        </select>
      </Field>
      <Field label="CGPA (out of 10)">
        <input className={inputClass} type="number" step="0.1" min="0" max="10"
      ...
      <Field label="Work experience (months)">
        <input className={inputClass} type="number" min="0"
          value={data.work_experience_months} onChange={e => update('work_experience_months', e.target.value)} placeholder="0" />
      </Field>
    </div>
  )
}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="IELTS overall band">
          <input className={inputClass} type="number" step="0.5" min="0" max="9"
        ...
        <Field label="TOEFL score">
          <input className={inputClass} type="number" min="0" max="120"
      ...
      <div className="mt-4 p-3 bg-indigo-50 rounded-lg text-xs text-indigo-700">
        💡 For Germany & most of Europe, IELTS is enough. GRE is mainly needed for US/Canada.
      </div>
    </div>
  )
}
...
// STEP 3: Preferences
// ===================
function Step3({ data, update, toggleCountry }: {
  data: FormData
  ...
  return (
    <div className="flex flex-col gap-4">
      <Field label="Total budget (₹ per year, including living)">
        <input className={inputClass} type="number"
          value={data.budget_inr} onChange={e => update('budget_inr', e.target.value)} placeholder="3000000" />
        {data.budget_inr && (
          <p className="text-xs text-gray-400 mt-1">
            ≈ ${Math.round(parseInt(data.budget_inr) / 83).toLocaleString()} USD/year
          </p>
        )}
      </Field>
      <Field label="Target intake">
        <select className={selectClass} value={data.target_intake} onChange={e => update('target_intake', e.target.value)}>
          <option value="">Select intake</option>
          {INTAKES.map(i => <option key={i}>{i}</option>)}
        </select>
      </Field>
      <Field label="Preferred countries (pick all that interest you)">
...
export default function ProfilePage() {
  ...
  // ← new: load existing profile
  useEffect(() => {
    if (!user) return
    supabase.from('profiles').select('*').eq('clerk_user_id', user.id).single()
      .then(({ data: profile }) => {
        if (profile) {
          setIsEdit(true)
          setData({
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const { error: dbError } = await supabase
        .from('profiles').upsert(payload, { onConflict: 'clerk_user_id' })
  ...
  if (loading) return <div className="text-center py-16 text-sm text-gray-400">Loading your profile...</div>

  return (
    <div className="max-w-xl mx-auto px-6 py-10">
      <h1 className="text-xl font-semibold text-gray-900 mb-1">
        {isEdit ? 'Edit your profile' : 'Complete your profile'}   {/* ← shows edit vs create */}
      </h1>
      <p className="text-sm text-gray-500 mb-6">
        {isEdit ? 'Update your details to refresh university recommendations.' : 'We use this to suggest universities, scholarships, and a roadmap.'}
      </p>
      ...
      <div className="flex gap-3">
        ...
        {step < 3 ? (
          <button onClick={() => setStep(s => s + 1)}
            className="flex-[2] px-4 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">
            Next →
          </button>
        ) : (
          <button onClick={saveProfile} disabled={saving}
            className="flex-[2] px-4 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-60">
